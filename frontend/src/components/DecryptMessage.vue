    ...
    <div class="shadow-xl rounded-xl p-6 md:p-10 max-w-5xl mx-auto bg-base-100">
      ...
      <div v-else>
        <div v-if="decryptedMessage" class="space-y-6">
          ...
          <div class="alert alert-info shadow-lg mt-8" v-if="messageData?.burn_after_reading">
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            <div class="flex flex-col gap-1">
              ...
              <span class="text-sm opacity-90"
                >The decrypted message will be deleted after refreshing the page for your
                security.</span
              >
            </div>
          </div>
    ...
    methods: {
      ...
      async fetchAndDecrypt() {
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/decrypt/${this.messageId}`
          );
          ...
          const encryptedBytes = Uint8Array.from(this.fromBase64UrlSafe(result.message), c =>
            c.charCodeAt(0)
          );
      ...
      async deleteMessage() {
        try {
          ...
          const response = await fetch(
            `${import.meta.env.VITE_API_BASE_URL}/delete/${this.messageId}`,
            {
              method: 'DELETE',
              headers: headers,
            }
